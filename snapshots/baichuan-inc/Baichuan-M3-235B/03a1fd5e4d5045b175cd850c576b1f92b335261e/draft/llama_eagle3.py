...
class LlamaDecoderLayer(LlamaDecoderLayer):
    def __init__(
        ...
        # Add gate_attention support
        self.attn_output_gate = getattr(config, "attn_output_gate", True)
        if self.attn_output_gate:
            logger.warning_once("using attn output gate!")

        # Calculate sizes for gate support
        self.q_size = self.self_attn.num_heads * self.self_attn.head_dim
        self.kv_size = self.self_attn.num_kv_heads * self.self_attn.head_dim

        # override qkv with gate support
        self.self_attn.qkv_proj = QKVParallelLinear(
            2 * self.hidden_size,
            self.self_attn.head_dim,
            self.self_attn.total_num_heads * (1 + self.attn_output_gate),
    ...
    def self_attention(
        ...
        if self.attn_output_gate:
            ...
            q_gate = q_gate.view(*orig_shape, self.self_attn.num_heads, -1)

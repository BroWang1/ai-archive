...
class RealtimeVenusOmniBatchFeature(BatchFeature):
    ...
    def to(self, *args, **kwargs) -> "RealtimeVenusOmniBatchFeature":
...
class RealtimeVenusOmniImageProcessor(BaseImageProcessor):
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        return RealtimeVenusOmniBatchFeature(
            data={"pixel_values": new_images_list, "image_sizes": image_sizes_list, "tgt_sizes": tgt_sizes_list},
            tensor_type=return_tensors,
        )


AutoImageProcessor.register("RealtimeVenusOmniImageProcessor", RealtimeVenusOmniImageProcessor)
...
class RealtimeVenusOmniAudioProcessor(WhisperFeatureExtractor):
    """
    ...
    def set_spac_log_norm(
        ...
        *,
        inplace: bool = True,
    ) -> "RealtimeVenusOmniAudioProcessor":
    ...
    def _np_extract_fbank_features(self, waveform_batch: np.ndarray, device: str) -> np.ndarray:
        """Extract NumPy log-Mel features with dynamic or fixed lower clipping limits."""
        if device != "cpu":
            raise ValueError(
        ...
        for waveform in waveform_batch:
            ...
            # Drop the final spectrogram frame.
            log_spec = log_spec[:, :-1]
...
class StreamingMelProcessorExact:
    ...
    def __init__(
        self,
        feature_extractor: RealtimeVenusOmniAudioProcessor,
        chunk_ms: int = 100,
    ...
    def _extract_full(self) -> torch.Tensor:
        ...
        if len(self.buffer) < 5 * self.sample_rate:
            # Use a fixed normalization floor for short audio buffers.
...
class RealtimeVenusOmniProcessor(ProcessorMixin):
    ...
    def process_image(
        ...
        Returns:
            RealtimeVenusOmniBatchFeature object
        """
        if images is None:
            return RealtimeVenusOmniBatchFeature(data={"pixel_values": [[]], "image_sizes": [[]], "tgt_sizes": [[]]})
        ...
        return RealtimeVenusOmniBatchFeature(data=model_inputs)
    ...
    def process_audio(
        ...
        """
        if audios is None:
            return RealtimeVenusOmniBatchFeature(data={"audio_features": [], "audio_feature_lens": []})
    ...
    def process_audio_streaming(
    ...
    ) -> Union[Tuple[torch.Tensor, dict], RealtimeVenusOmniBatchFeature]:
        ...
        Args:
            ...
            return_batch_feature: whether to return RealtimeVenusOmniBatchFeature format (consistent with process_audio)
        Returns:
            ...
            If return_batch_feature=True:
                RealtimeVenusOmniBatchFeature object, containing:
                - audio_features: [1, 80, n_frames] mel features
    ...
    def __call__(
        ...
        **kwargs,
        ...
        result = RealtimeVenusOmniBatchFeature(data={**model_inputs})

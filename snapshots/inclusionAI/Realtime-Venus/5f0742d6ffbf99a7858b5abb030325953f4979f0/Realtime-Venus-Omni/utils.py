...
class SpeculativeSnapshot:
    ...
    # Checksums for validating restored state
...
class TTSStreamingGenerator:
    ...
    def generate_with_buffer(
        ...
        for t in range(max_new_token):
            ...
            if not audio_bos:
                # use generated tokens (current chunk) as input for processor/warper (align with modeling_realtime_venus_omni)
...
def as_dynamic_cache(past_key_values):
    """Convert a tuple cache to DynamicCache if needed."""
...
def normalize_content_item(item) -> Union[str, Any, List[Any]]:
    ...
    if isinstance(item, dict):
        ...
        if item_type == "text":
            ...
            from realtime_venus_omni.utils import get_video_frame_audio_segments

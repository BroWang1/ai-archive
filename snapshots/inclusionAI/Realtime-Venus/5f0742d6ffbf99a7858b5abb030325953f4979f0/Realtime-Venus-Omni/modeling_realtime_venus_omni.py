...
# Modifications Copyright 2026 The Realtime-Venus Team.
#
...
from .configuration_realtime_venus_omni import RealtimeVenusOmniConfig
from .configuration_realtime_venus_omni import RealtimeVenusOmniTTSConfig
from .modeling_navit_siglip import SiglipVisionTransformer
from .processing_realtime_venus_omni import RealtimeVenusOmniProcessor
...
class RealtimeVenusOmniPreTrainedModel(Qwen3PreTrainedModel):
    config_class = RealtimeVenusOmniConfig


class RealtimeVenusOmni(RealtimeVenusOmniPreTrainedModel):
    def __init__(self, config):
        ...
        self.force_rope_reindex = False  # Force rotary position reindexing
    ...
    def init_audio_module(self):
        ...
        return RealtimeVenusOmniWhisperEncoder(self.config.audio_config)
    ...
    def init_tts_module(self):
        ...
        return RealtimeVenusOmniTTS(config=self.config.tts_config, audio_tokenizer=None)
    ...
    def _default_reference_audio_path(self) -> Path:
        """Find the reference voice in the loaded model's assets, without downloading files."""
        config = getattr(self, "config", None)
        source = getattr(config, "_name_or_path", None)
        if not isinstance(source, str) or not source.strip():
            raise ValueError("Cannot locate the default voice: model.config._name_or_path is empty")
        model_dir = Path(source).expanduser()
        if model_dir.is_dir():
            candidates = (model_dir / "assets/HT_ref_audio.wav",)
        elif not model_dir.is_absolute() and not source.startswith("."):
            ...
            candidates = (Path(cached),) if isinstance(cached, str) else ()
        else:
            candidates = ()
        for path in candidates:
            if path.is_file():
                ...
                if not header or header.startswith(b"version https://git-lfs.github.com/spec/"):
                    raise ValueError(f"Default reference audio is empty or an LFS pointer: {path}")
                return path.resolve()
        raise FileNotFoundError(
            "Default reference audio is missing. Place HT_ref_audio.wav in the model's "
            "assets/ directory, or pass a reference explicitly. No audio was downloaded."
        )

    def _load_default_reference_audio(self) -> tuple[np.ndarray, str]:
        """Load the packaged voice as finite, non-empty mono audio at 16 kHz."""
        path = self._default_reference_audio_path()
        import librosa

        waveform, _ = librosa.load(str(path), sr=16000, mono=True)
        if waveform.ndim != 1 or waveform.size == 0 or not np.isfinite(waveform).all():
            raise ValueError(f"Default reference audio must contain finite mono samples: {path}")
        return waveform, str(path)

    def _with_default_omni_system(self, msgs: Any, generate_audio: bool) -> Any:
        """Add a default system message only to conversations that do not provide one."""
        decoded = json.loads(msgs) if isinstance(msgs, str) else msgs
        if not isinstance(decoded, list) or not decoded:
            raise ValueError("Omni Chat msgs must be a non-empty conversation or batch")
        batched = isinstance(decoded[0], list)
        conversations = decoded if batched else [decoded]
        for conversation in conversations:
            if not conversation or not all(isinstance(item, dict) for item in conversation):
                raise ValueError("Each Omni Chat conversation must contain message dictionaries")
        missing = [
            not any(item.get("role") == "system" for item in conversation)
            for conversation in conversations
        ]
        if not any(missing):
            return msgs
        reference = self._load_default_reference_audio()[0] if generate_audio else None
        system = self.get_sys_prompt(ref_audio=reference, mode="omni")
        prepared = [
            [system, *conversation] if needs_system else conversation
            for conversation, needs_system in zip(conversations, missing)
        ]
    ...
    @staticmethod
    def get_sys_prompt(
        ref_audio=None, mode="default", language="en", ref_audio_max_ms=None, system_prompt=None
    ):
        """Build a system message with the default system prompt and optional voice reference."""
        ...
        if mode == "omni":
            ...
            if language == "zh":
                ...
                vc_prompt_suffix = system_prompt + "\n\n你的任务是用这种声音模式来当一个助手。请认真、高质量地回复用户的问题。请用高自然度的方式和用户聊天。"
            else:
                vc_prompt_prefix = "Clone the voice in the provided audio prompt."
                vc_prompt_suffix = system_prompt + "\n\nPlease assist users while maintaining this voice style. Please answer the user's questions seriously and in a high quality. Please chat with the user in a highly human-like and oral style."
            ...
            if language == "zh":
                vc_prompt_prefix = system_prompt + "\n\n模仿输入音频中的声音特征。"
    ...
    def prepare_processor(self, processor=None, tokenizer=None):
        ...
        if not hasattr(self, "processor") or self.processor is None:
            self.processor = RealtimeVenusOmniProcessor.from_pretrained(self.config._name_or_path, trust_remote_code=True)
        if tokenizer is not None:
            self.processor.tokenizer = tokenizer

    def use_memory(self, *, memory_minutes: float | None = None) -> "RealtimeVenusOmni":
        """Enable Memory for Chat and Duplex on this model instance.

        Chat retrieves relevant content from the complete input media. Duplex
        maintains a live stream with historical memory for answering questions.

        memory_minutes sets the Duplex archive retention period (default 10
        minutes). Configure it before calling as_duplex(). The retention period
        does not limit video duration or change the live window.
        """
        from .realtime_venus_omni_memory import configure_memory

        configure_memory(self, memory_minutes=memory_minutes)
        return self
    ...
    def chat(
        ...
        **kwargs,
        ...
        """Run Chat with an optional input-length override.

        Memory Chat defaults to the configured LLM context length for token
        accounting, without a pre-generation capacity check. Chat without Memory
        retains the 8192-token input limit.
        """
        if omni_mode:
            msgs = self._with_default_omni_system(msgs, generate_audio)
        memory_controller = getattr(self, "_realtime_venus_omni_memory_controller", None)
        if max_inp_length is None:
            max_inp_length = (
                self.llm.config.max_position_embeddings
                if memory_controller is not None else 8192
            )
        if memory_controller is not None:
            return memory_controller.chat(dict(locals()))
            ...
            for i, msg in enumerate(copy_msgs):
                ...
                for c in content:
                    if isinstance(c, Image.Image):
                        ...
                        # Select the TTS template through the use_tts_template parameter.
        ...
        if not teacher_forcing and use_tts_template and generate_audio and output_audio_path:
            stop_ids = {
                self.processor.tokenizer.convert_tokens_to_ids(t)
                for t in self.terminators
            }
            stop_ids.add(tts_eos_token)
            ids = full_sequence.tolist()
            tts_eos_idx = next(
                (i for i in range(max(tts_bos_idx, 0), len(ids)) if ids[i] in stop_ids),
                None,
            )
            if tts_bos_idx < 0 or tts_eos_idx is None or tts_eos_idx <= tts_bos_idx:
                raise RuntimeError("Invalid Chat TTS text boundary")
    ...
    # Rebuild the streaming cache from retained input token IDs.
        ...
        **kwargs,
        ...
        def audio_chunk_generator(
            ...
            **kwargs,
            ...
            if teacher_forcing:
                # Prefill bos_input using the KV cache retained by streaming_prefill.
    ...
    def as_duplex(self, device: Optional[str] = None, **kwargs) -> "RealtimeVenusOmniDuplex":
        """Convert this RealtimeVenusOmni instance to RealtimeVenusOmniDuplex for full-duplex streaming."""
        memory_controller = getattr(self, "_realtime_venus_omni_memory_controller", None)
        if memory_controller is not None:
            kwargs = memory_controller.duplex_factory_kwargs(kwargs)
        official_duplex = RealtimeVenusOmniDuplex.from_existing_model(
            model=self,
            device=device,
            **kwargs,
        )
        if memory_controller is None:
            return official_duplex
        return memory_controller.wrap_duplex(official_duplex)


class RealtimeVenusOmniDuplex:
    """Stream audio and video input while generating text and speech.

    Use RealtimeVenusOmni.as_duplex() to create an instance from a loaded model.
    """
    ...
    def from_existing_model(
        cls,
        model: "RealtimeVenusOmni",
        device: Optional[str] = None,
        **kwargs,
    ) -> "RealtimeVenusOmniDuplex":
        """Create RealtimeVenusOmniDuplex from an existing RealtimeVenusOmni instance."""
        ...
        if instance.processor is None:
            ...
            instance.processor = RealtimeVenusOmniProcessor.from_pretrained(instance.name_or_path, trust_remote_code=True)
    ...
    def _reset_streaming_state(self):
        self._prepared_prompt_wav_path = None
        self.audio_chunk_idx = 0
        ...
        # <delegate> span masking state (payload must not be spoken by the talker)
        self._delegate_tts_skip = False
        # set when a fully-muted delegate chunk already flushed the turn's speech
        self._delegate_turn_preflushed = False
    ...
    def prepare(
        ...
        **kwargs,
        ...
        prefix_system_prompt = prefix_system_prompt or DEFAULT_SYSTEM_PROMPT
        if self.generate_audio and ref_audio is None and prompt_wav_path is None:
            ref_audio, prompt_wav_path = self.model._load_default_reference_audio()
        ...
        self._prepared_prompt_wav_path = prompt_wav_path
        self.decoder.reset()
        ...
        if has_text:
            ...
            if len(text_token_ids) > 0:
                ...
                if mode == "TEXT":
                    ...
                    # mixed mode: text is the last content fed in this unit; take the
                    # decision logits after it so injected text (question / <backend>)
                    # can trigger this second's speak, matching the training format.
                    self.pending_logits, _ = self.decoder.feed(text_embeds, return_logits=True)
    ...
    def streaming_generate(
        ...
        if tts_condition is None:
            # every token of this second was inside a <delegate>...</delegate>
            # span -> keep the unit in the text stream but speak nothing
            audio_waveform = self._generate_silence_waveform()
            if self.tts_past_key_values is not None:
                # Mid-turn muted chunk: flush the turn's remaining speech NOW (as
                # if the closing turn_eos had arrived one chunk early), so the
                # speech tail is not delayed behind silent chunks. The real
                # end_of_turn chunk skips TTS via _delegate_turn_preflushed.
                ...
                flushed = self._generate_waveform_from_tokens(
                    new_tokens, prompt_wav_path, True, force_flush=True
                )
                self.tts_text_start_pos = 0
                self.tts_past_key_values = None
                self.tts_current_turn_start_time = None
                self._reset_token2wav_for_new_turn()
                if not end_of_turn:
                    self._delegate_turn_preflushed = True
                if flushed is not None:
                    audio_waveform = flushed
            elif end_of_turn:
                self._delegate_turn_preflushed = False
            return {
                "is_listen": False,
        ...
        if (end_of_turn and not generated_text.strip()
                and getattr(self, "_delegate_turn_preflushed", False)):
            # the turn's speech was already flushed at the fully-muted delegate
            # chunk -> skip TTS for this terminator-only chunk and just close the
            # turn cleanly; the next turn starts from fresh state
            self._delegate_turn_preflushed = False
            return {
                "is_listen": False,
                "text": generated_text,
    ...
    def _convert_results_to_tts_input(self, results):
        ...
        # Mask <delegate>...</delegate> spans: the delegation payload reaches the
        # backend through the text stream only and must never be spoken by the
        # talker. The skip flag persists across seconds (this method is called
        # once per second); it is reset per turn in _reset_streaming_state, and
        # force-cleared at end_of_turn in case </delegate> is never emitted.
        if not hasattr(self, "_delegate_open_id"):
            self._delegate_open_id = self.tokenizer.convert_tokens_to_ids("<delegate>")
            self._delegate_close_id = self.tokenizer.convert_tokens_to_ids("</delegate>")
            if self._delegate_open_id is None or self._delegate_close_id is None:
                self._delegate_open_id = None  # tokens missing -> masking disabled
        if self._delegate_open_id is not None and (
                getattr(self, "_delegate_tts_skip", False)
                or any(h[0] == self._delegate_open_id for h in results)):
            had_end_of_turn = any(h[2] for h in results)
            kept = []
            for h in results:
                tid = h[0]
                if tid == self._delegate_open_id:
                    self._delegate_tts_skip = True
                    continue
                if tid == self._delegate_close_id:
                    self._delegate_tts_skip = False
                    continue
                if self._delegate_tts_skip:
                    continue
                kept.append(h)
            if had_end_of_turn and self._delegate_tts_skip:
                self._delegate_tts_skip = False  # unclosed span: don't mute the next turn
            results = kept
            if len(results) == 0:
                return None  # nothing speakable this second -> caller skips TTS
        if len(results) == 0:
            audio_bos = self.model.tts.emb_text(
                torch.tensor(
                    [self.model.tts.audio_bos_token_id],
    ...
    def _generate_waveform_from_tokens(
        ...
        if prompt_wav_path is None:
            prompt_wav_path = getattr(self, "_prepared_prompt_wav_path", None)
    ...
    def as_simplex(self, reset_session: bool = True, reset_token2wav_cache: bool = False) -> "RealtimeVenusOmni":
        """Convert this RealtimeVenusOmniDuplex instance back to RealtimeVenusOmni for simplex mode.

        Args:
            reset_session: If True, reset streaming session state (KV cache, etc.).
                          Recommended when switching from duplex to simplex mode.

        Returns the underlying RealtimeVenusOmni model instance without reloading.
        """
        if reset_session:
...
class RealtimeVenusOmniWhisperEncoderLayer(nn.Module):
...
class RealtimeVenusOmniWhisperEncoder(WhisperEncoder):

    def __init__(self, config: WhisperConfig):
        super().__init__(config)
        self.layers = nn.ModuleList(
            [RealtimeVenusOmniWhisperEncoderLayer(config, layer_idx=i) for i in range(config.encoder_layers)]
        )
...
class RealtimeVenusOmniMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
@dataclass
class RealtimeVenusOmniTTSGenerationOutput(ModelOutput):
    """
    Output class for RealtimeVenusOmniTTS generation.
...
class RealtimeVenusOmniTTS(PreTrainedModel):
    config_class = RealtimeVenusOmniTTSConfig

    def __init__(self, config: RealtimeVenusOmniTTSConfig, audio_tokenizer: None):
        super().__init__(config)
    ...
    def create_projector(config):
        if config.projector_type == "mlp":
            ...
            return RealtimeVenusOmniMLP(config)
    ...
    def generate(
        ...
        return RealtimeVenusOmniTTSGenerationOutput(
    ...
    def interleaved_generate(
        ...
        **kwargs,
        ...
        return RealtimeVenusOmniTTSGenerationOutput(new_ids=torch.cat(generated_tokens, dim=1), finished=True)

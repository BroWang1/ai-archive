...
# The MoE experts are kept fused in memory (`gate_up_proj` / `down_proj` 3D
# tensors) for an efficient grouped forward, but the checkpoint stores each
# expert separately as `...mlp.experts.{i}.{gate,up,down}_proj.weight`. Mirroring
# the upstream Qwen3-MoE implementation, we register a bidirectional
# `WeightConverter`: it merges the sharded expert weights into the fused
# parameters on load, and `revert_weight_conversion` (called from
# `save_pretrained`) splits them back into the sharded layout on save.
_INTERN_S2_EXPERTS_CONVERSIONS = [
    WeightConverter(
        source_patterns=[
...
# Lookup is by class name first, then `config.model_type`; register under both
# the multimodal model type and the text model type so the conversion applies
# whether the full model or the text backbone is loaded standalone.
for _model_type in ("intern_s2_preview", "qwen3_5_moe_text"):
    register_checkpoint_conversion_mapping(_model_type, _INTERN_S2_EXPERTS_CONVERSIONS, overwrite=True)
